...
async def _completions_via_chat(request: Request, form_data: dict, user, requested_model: str):
    """
    ...
    if isinstance(response, StreamingResponse):
        ...
        return convert_chat_to_completions_response(response, model=requested_model, echo_prompt=echo_prompt)
    else:
        # Passthrough for error responses (JSONResponse, PlainTextResponse, etc.)
        return response
